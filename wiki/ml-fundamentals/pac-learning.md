...
**Summary**: The Probably Approximately Correct (PAC) framework formalizes the question "how many training examples do we need to learn a good classifier with high confidence?" It connects finite-class bounds, agnostic learning, uniform convergence, and the role of inductive bias.

**Course**: ml-fundamentals

**Sources**: ls26_lecture_pac.pdf, ls26_seminar_pac_solutions.pdf, understanding-machine-learning-theory-algorithms.pdf
...
In the realizable PAC model, the target labels are assumed to be generated by some $h \in \mathcal{H}$; agnostic PAC learning drops this assumption and only asks the learner to compete with the best hypothesis in $\mathcal{H}$ (source: ml-fundamentals/understanding-machine-learning-theory-algorithms.pdf). The lecture definition above is in the agnostic spirit because it bounds excess risk relative to $h_{\mathcal{H}}$ rather than requiring zero Bayes error inside the class.

## Sample Complexity for Finite $\mathcal{H}$

For a finite hypothesis class with $|\mathcal{H}|$ hypotheses and 0/1-loss, the ERM algorithm is a successful PAC learner with sample complexity for excess risk at most $\varepsilon$ (source: ml-fundamentals/ls26_lecture_pac.pdf):
...
This bound is the first formal version of "ERM plus inductive bias works": if $\mathcal{H}$ is finite, a sufficiently large sample makes empirical risk uniformly close to true risk for every $h \in \mathcal{H}$, so choosing the empirical minimizer is safe (source: ml-fundamentals/understanding-machine-learning-theory-algorithms.pdf).

## Uniform Convergence View

Uniform convergence is the event that empirical and true risks are close **simultaneously** for all hypotheses in $\mathcal{H}$:

$$\forall h \in \mathcal{H}, \quad |\hat{R}(\mathcal{T}_m,h) - R(p,h)| \leq \varepsilon$$

If this holds, then ERM generalizes: the empirical minimizer cannot be much worse than the best-in-class hypothesis, because both the learned hypothesis and the comparator have empirical risks close to their true risks (source: ml-fundamentals/understanding-machine-learning-theory-algorithms.pdf). For finite classes, Hoeffding's inequality plus the union bound proves uniform convergence; for infinite binary classes, [[ml-fundamentals/vc-dimension]] gives the exact combinatorial condition (source: ml-fundamentals/understanding-machine-learning-theory-algorithms.pdf).
